defmodule Common.S3Test do
  ...
  describe "list_keys/2" do
    test "returns stream of keys on success" do
      ...
      with_mocks [
        {ExAws, [:passthrough], [stream!: fn _ -> mock_objects end]},
        {ExAws.S3, [:passthrough], [list_objects_v2: fn _, _ -> :mocked_call end]}
      ] do
    ...
    test "passes opts to list_objects_v2" do
      ...
      with_mocks [
        {ExAws, [:passthrough], [stream!: fn _ -> [] end]},
        {ExAws.S3, [:passthrough], [list_objects_v2: fn _, _ -> :mocked_call end]}
      ] do
    ...
    test "reraises exception on error" do
      ...
      with_mock ExAws, [:passthrough], stream!: fn _ -> raise "S3 error" end do
  ...
  describe "get_object/2" do
    test "returns stream of object data on success" do
      ...
      with_mocks [
        {ExAws, [:passthrough], [stream!: fn _ -> mock_data end]},
        {ExAws.S3, [:passthrough], [download_file: fn _, _, _ -> :mocked_call end]}
      ] do
    ...
    test "reraises exception on error" do
      ...
      with_mock ExAws, [:passthrough], stream!: fn _ -> raise "Download error" end do
  ...
  describe "list_objects/2" do
    test "returns {:ok, objects} on success" do
      ...
      with_mocks [
        {ExAws, [:passthrough], [request: fn _ -> {:ok, mock_response} end]},
        {ExAws.S3, [:passthrough], [list_objects_v2: fn _, _ -> :mocked_call end]}
      ] do
    ...
    test "passes opts to list_objects_v2" do
      ...
      with_mocks [
        {ExAws, [:passthrough], [request: fn _ -> {:ok, %{contents: []}} end]},
        {ExAws.S3, [:passthrough], [list_objects_v2: fn _, _ -> :mocked_call end]}
      ] do
    ...
    test "returns {:error, reason} on AWS error" do
      ...
      with_mock ExAws, [:passthrough], request: fn _ -> {:error, error_reason} end do
        result = S3.list_objects(bucket)
    ...
    test "returns {:error, message} on exception" do
      ...
      with_mock ExAws, [:passthrough], request: fn _ -> raise "Network error" end do
        result = S3.list_objects(bucket)
