defmodule Etl.Enricher do
  ...
  def enrich(sample) do
    with :ok <- validate_input_sample(sample),
         ...
         row when not is_nil(row) <-
           Etl.DestinationCache.get_row(sample.shipper) do
  ...
  defp get_api_url do
    Application.get_env(:tnt_pipeline, :api_url) ||
  ...
  defp get_config(key, default \\ nil) do
    case Application.get_env(:tnt_pipeline, key) do
  ...
  defp validate_auth_config do
    ...
    missing_configs =
      required_configs
      |> Enum.filter(fn key ->
        Application.get_env(:tnt_pipeline, key) |> is_nil()
      end)
